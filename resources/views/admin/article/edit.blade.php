        ...
        <section class="boxs">
            <div class="boxs-header">
                <h3 class="custom-font hb-cyan">
                    <strong>修改</strong>文章</h3>
            </div>

            <div class="boxs-body">
                <form class="form-horizontal" role="form" action="{{ route('admin.article.update',['id'=>$articleData,'url'=>$url_query]) }}" method="post">
                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">文章标题</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="inputEmail3" name="title" value="{{ $articleData->title }}">
                        </div>
                    </div>
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-sm-2 col-xs-6">
                            <select class="form-control inline" name="cid">
                                @foreach($cateData as $val)
                                    <option value="{{ $val['id'] }}" @if($articleData->cates->id == $val['id'] ) selected @endif>{{ $val['cname'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="inputEmail3" name="desn" value="{{ $articleData->desn }}">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="imgbox" style="display: block;">
                            <span class="delimg" onclick="delimg('{{$articleData->id}}','{{ $articleData->pic }}')">
                                X
                            </span>
                            <img src="{{ $articleData->pic }}"  id="showpic">
                        </div>
                    </div>
                    <input type="hidden" id="pic" value="{{ $articleData->pic }}" name="pic">
                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">文章内容</label>
                    </div>
                    <textarea name="body" id="editor" style="margin-left: 160px;">{{ $articleData->body }}</textarea>
                    <div class="form-group">
